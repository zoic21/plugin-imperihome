...
class imperihome {
		...
		if (is_array($return['devices'])) {
			foreach ($return['devices'] as &$device) {
				...
				foreach ($device['params'] as &$param) {
					...
					if (isset($param['invertState']) && $param['invertState'] == 1) {
	...
	public static function generateParam($cmd, $cmdType, $ISSStructure) {
		...
		foreach ($return['params'] as &$param) {
			...
			if (isset($param['invertState']) && $cmd->getDisplay('invertBinary', 0) == 1) {
				$param['invertState'] = 1;
			}
			if ($cmdType == 'DevDoor' && isset($param['invertState']) && $cmd->getDisplay('invertBinary', 0) == 0) {
				$param['invertState'] = 1;
			} else {
				$param['invertState'] = 0;
			}
